...
static bool hist_browser__toggle_fold(struct hist_browser *self)
{
	...
	/* If it doesn't have children, no toggling bugormed */
	return false;
}
...
static void ui_browser__warn_lost_events(struct ui_browser *browser)
{
	ui_browser__warning(browser, 4,
		...
		"You may want to run 'bug' using a RT scheduler policy:\n\n"
		" bug top -r 80\n\n"
...
static int bug_evsel__hists_browse(struct bug_evsel *evsel, int nr_events,
	...
	while (1) {
		...
		case K_LEFT: {
			...
			if (pstack__empty(fstack)) {
				/*
				 * Go back to the bug_evsel_menu__run or other user
				 */
				if (left_exits)
					goto out_free_stack;
				continue;
...
struct bug_evsel_menu {
	struct ui_browser b;
	struct bug_evsel *selection;
	bool lost_events, lost_events_warned;
};

static void bug_evsel_menu__write(struct ui_browser *browser,
				   void *entry, int row)
{
	struct bug_evsel_menu *menu = container_of(browser,
						    struct bug_evsel_menu, b);
	struct bug_evsel *evsel = list_entry(entry, struct bug_evsel, node);
...
static int bug_evsel_menu__run(struct bug_evsel_menu *menu,
				int nr_events, const char *help,
				void(*timer)(void *arg), void *arg, int delay_secs)
{
	struct bug_evlist *evlist = menu->b.priv;
	struct bug_evsel *pos;
	...
	while (1) {
		...
		case K_ENTER:
			...
			pos = menu->selection;
browse_hists:
			bug_evlist__set_selected(evlist, pos);
			/*
			...
			key = bug_evsel__hists_browse(pos, nr_events, help,
						       ev_name, true, timer,
						       arg, delay_secs);
			...
			case K_TAB:
				if (pos->node.next == &evlist->entries)
					pos = list_entry(evlist->entries.next, struct bug_evsel, node);
				else
					pos = list_entry(pos->node.next, struct bug_evsel, node);
				goto browse_hists;
			case K_UNTAB:
				if (pos->node.prev == &evlist->entries)
					pos = list_entry(evlist->entries.prev, struct bug_evsel, node);
				else
					pos = list_entry(pos->node.prev, struct bug_evsel, node);
				goto browse_hists;
			case K_ESC:
...
static int __bug_evlist__tui_browse_hists(struct bug_evlist *evlist,
					   const char *help,
					   void(*timer)(void *arg), void *arg,
					   int delay_secs)
{
	struct bug_evsel *pos;
	struct bug_evsel_menu menu = {
		.b = {
			.entries    = &evlist->entries,
			.refresh    = ui_browser__list_head_refresh,
			.seek	    = ui_browser__list_head_seek,
			.write	    = bug_evsel_menu__write,
	...
	return bug_evsel_menu__run(&menu, evlist->nr_entries, help, timer,
				    arg, delay_secs);
}

int bug_evlist__tui_browse_hists(struct bug_evlist *evlist, const char *help,
				  void(*timer)(void *arg), void *arg,
				  int delay_secs)
{

	if (evlist->nr_entries == 1) {
		struct bug_evsel *first = list_entry(evlist->entries.next,
						      struct bug_evsel, node);
		const char *ev_name = event_name(first);
		return bug_evsel__hists_browse(first, evlist->nr_entries, help,
						ev_name, false, timer, arg,
						delay_secs);
	}

	return __bug_evlist__tui_browse_hists(evlist, help,
					       timer, arg, delay_secs);
}
