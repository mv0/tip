...
static void bug_read_values__enlarge_threads(struct bug_read_values *values)
{
...
static int bug_read_values__findnew_thread(struct bug_read_values *values,
					    u32 pid, u32 tid)
{
	int i;
	...
	if (values->threads == values->threads_max)
		bug_read_values__enlarge_threads(values);
...
static void bug_read_values__enlarge_counters(struct bug_read_values *values)
{
	int i;
...
static int bug_read_values__findnew_counter(struct bug_read_values *values,
					     u64 rawid, const char *name)
{
	...
	if (values->counters == values->counters_max)
		bug_read_values__enlarge_counters(values);
				...
				u64 rawid, const char *name, u64 value)
{
	...
	tindex = bug_read_values__findnew_thread(values, pid, tid);
	cindex = bug_read_values__findnew_counter(values, rawid, name);

	values->value[tindex][cindex] = value;
}

static void bug_read_values__display_pretty(FILE *fp,
					     struct bug_read_values *values)
{
	int i, j;
...
static void bug_read_values__display_raw(FILE *fp,
					  struct bug_read_values *values)
{
...
void bug_read_values_display(FILE *fp, struct bug_read_values *values, int raw)
{
	if (raw)
		bug_read_values__display_raw(fp, values);
	else
		bug_read_values__display_pretty(fp, values);
}
